...
/* Remove entries matching ALL provided filters (name/score/at). Used by the
   per-entry DELETE so a single bad score can be dropped without wiping the
   board. Filters with undefined values are ignored. */
async function remove(filters) {
  const list = await load();
  const keep = list.filter((e) => {
    if (filters.name !== undefined && e.name !== filters.name) {
      return true;
    }
    if (filters.score !== undefined && e.score !== filters.score) {
      return true;
    }
    if (filters.at !== undefined && e.at !== filters.at) {
      return true;
    }
    return false;
  });
  const next = keep.slice(0, LIMIT);
  await save(next);
  return next;
}
...
module.exports = { load, add, remove, clear, cleanName, LIMIT, debugBackend, redisPing };
